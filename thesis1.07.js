...
(function () {
    ...
    // 内部工具函数：去除常见水印参数（如 watermark、wm、stamp）
    ...
    const PLATFORMS = {
        ...
        // 南开大学（支持 webvpn 和直连 thesis.nankai.edu.cn）
        NANKAI: {
            name: '南开大学',
            match: (url) => url.includes('webvpn.nankai.edu.cn') || url.includes('thesis.nankai.edu.cn'),
            chunksPerPage: 3,
            getParams: function () {
                const url = new URL(window.location.href);
                return {
            ...
            getJumpUrl: function (fid, fileId, page) {
                const currentUrl = window.location.href;
                if (currentUrl.includes('thesis.nankai.edu.cn')) {
