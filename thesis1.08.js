    ...
    const PLATFORMS = {
        ...
        NANKAI: {
            ...
            match: (url) => url.includes('webvpn.nankai.edu.cn') || 
                           url.includes('thesis.nankai.edu.cn') || 
                           url.includes('libvpn.nankai.edu.cn'),
            chunksPerPage: 3,
            // 提取 libvpn 基础路径
            getVpnBase: function () {
                const currentUrl = window.location.href;
                const match = currentUrl.match(/(https:\/\/libvpn\.nankai\.edu\.cn\/https\/[a-f0-9]+)/);
                if (match) return match[1];
                return null; // 非 libvpn 返回 null
            },
            getParams: function () {
                const url = new URL(window.location.href);
                return {
            ...
            getJumpUrl: function (fid, fileId, page) {
                ...
                if (vpnBase) {
                    // libvpn 模式
                    return `${vpnBase}/read/jumpServlet?vpn-1&page=${page}&fid=${fid}&fileId=${fileId}`;
                } else if (window.location.href.includes('thesis.nankai.edu.cn')) {
                    // 直连模式
                    return `https://thesis.nankai.edu.cn/read/jumpServlet?page=${page}&fid=${fid}&fileId=${fileId}`;
                } else {
                    // webvpn 模式
            ...
            processImageUrl: function (src) {
                const vpnBase = this.getVpnBase();
                if (!vpnBase) return src; // 非 libvpn 直接返回原链接
                // libvpn 模式：将 thesis.nankai.edu.cn 的图片地址转换为 libvpn 代理地址
                if (src && src.includes('thesis.nankai.edu.cn')) {
                    const pathMatch = src.match(/\/read\/pdfboxServlet\?.*$/);
                    if (pathMatch) {
                        let path = pathMatch[0];
                        path = path.replace('?', '?vpn-1&');
